...
bool FtpReader::View(const File *pFileOriginal, File *pFileChange) {
    ...
    _pProgress->init();
    _pProgress->setTitle("ftp view file");
...
bool FtpReader::Copy(Selection &tSelection,
    ...
    _pProgress->init();
    _pProgress->setTitle("Copy to " + sTargetPathTmp2);
...
bool FtpReader::Remove(Selection &tSelection, bool bMsgShow, bool bIgnore) {
    ...
    if (bMsgShow) {
        ...
        if (nCount && uSize)
            _pProgress->setOneValue((int) ((long) (nCount / uSize) / 100.0));
    }
...
bool FtpReader::Paste(Selection &tSelection) {
    ...
    _pProgress->init();
    _pProgress->setTitle(_("Copy to ") + sTargetPathTmp2);
