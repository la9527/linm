...
void Dialog_RemoteConnInsert::Execute(KeyInfo &tKeyInfo) {
    switch (_nCurNum) {
        case 0:
            InputExe(&_tName, tKeyInfo);
            return;
        case 1:
            CheckRadioExe(&_tSFTP, tKeyInfo, &_tFTP);
            return;
        case 2:
            CheckRadioExe(&_tFTP, tKeyInfo, &_tSFTP);
            return;
        case 3:
            CheckRadioExe(&_tSAMBA, tKeyInfo, &_tSFTP);
            return;
        case 4:
            InputExe(&_tAddr, tKeyInfo);
            return;
        case 5:
            InputExe(&_tDirectory, tKeyInfo);
            return;
        case 6:
            InputExe(&_tPort, tKeyInfo);
            return;
        case 7:
            CheckRadioExe(&_tAnonymous, tKeyInfo);
            return;
        case 8:
            CheckRadioExe(&_tKeyFile, tKeyInfo);
            return;
        case 9:
            InputExe(&_tID, tKeyInfo);
            return;
    }
    ...
    if (!_tKeyFile.GetCheck()) {
        switch (_nCurNum) {
            case 10:
                InputExe(&_tPasswd, tKeyInfo);
                break;
            case 11:
                ButtonExe(&_tOk, tKeyInfo);
                break;
            case 12:
                ButtonExe(&_tCancel, tKeyInfo);
                break;
        }
    } else {
        switch (_nCurNum) {
            case 10:
                InputExe(&_tPublicKey, tKeyInfo);
                break;
            case 11:
                InputExe(&_tPrivateKey, tKeyInfo);
                break;
            case 12:
                ButtonExe(&_tOk, tKeyInfo);
                break;
