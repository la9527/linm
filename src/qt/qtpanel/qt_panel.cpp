...
void	Qt_Panel::mouseMoveEvent(QMouseEvent* event)
{
	...
	{
		pFileBox = _vDrawFileList[n];
		if ( pFileBox && pFileBox->AreaChk(event->x(), event->y()) )
		{
			if ( pFileBox->GetFile() )
			{
...
void Qt_Panel::paintEvent(QPaintEvent */*event*/)
{
	...
	if ( hasFocus() )
	{
		...
		QPen 	pen( focusColor, 2, Qt::SolidLine, Qt::RoundCap, Qt::RoundJoin);
	...
	if ( _bAllDraw )
	{
		...
		FileImageCreate( viewBox );
		_bAllDraw = false;
	}
	
	for( int n = 0; n < _vDrawFileList.size(); n++ )
	{
		Qt_FileBox* 	pFileBox = _vDrawFileList[n];

		if ( !pFileBox ) continue;
		
		if ( pFileBox->GetNumber() == (int)_uCur && _bFocus )
		...
		if ( pFileBox->GetImage() )
		{
			//qDebug() << pFileBox->x << pFileBox->y << pFileBox->GetImage()->size();
			if ( pFileBox->GetFocus() )
				painter.drawImage( pFileBox->x, pFileBox->y, pFileBox->GetFocusImage() );
			else
				painter.drawImage( pFileBox->x, pFileBox->y, *pFileBox->GetImage() );
		}
	}

	for ( int n = 0; n < _vDrawTitleList.size(); n++ )
	{
		Qt_FileBox* 	pTitleBox = _vDrawTitleList[n];
		if ( pTitleBox->GetImage() )
			painter.drawImage( pTitleBox->x, pTitleBox->y, *pTitleBox->GetImage() );
	}
...
void Qt_Panel::FileImageCreate( const QRect& viewBox )
{
	int				nNum = 0;
	Qt_FileBox* 	pFileBox = NULL;
	...
	for (int nCol = 0; nCol < _nCol; nCol++)
	{
		for (int nRow = 0; nRow < _nRow; nRow++)
		{
			...
			pFileBox->height = _nDefBoxHeight - 1;
			pFileBox->width  = _nItemWidth - 1;
	...
	for (int nCol = 0; nCol < nColView+1; nCol++)
	{
		if (nCol < (int)_vDrawTitleList.size())
		{
			...
			pTitleBox->Show();
			//qDebug() << "Title Show ----> " << pTitleBox->x << pTitleBox->y << pTitleBox->width << pTitleBox->height;
		}
	}
}
...
void Qt_Panel::resizeEvent(QResizeEvent* )
{
	_bAllDraw = true;
	repaint();
}
...
void	Qt_Panel::ReadEnd()
{
	...
	for( int n = 0; n < (int)_vDirFiles.size(); n++ )
	{
		pFile = _vDirFiles[n];
		if ( pFile )
		{
