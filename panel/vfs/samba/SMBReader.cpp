...
bool SMBReader::Paste(Selection &tSelection) {
    ...
    for (int n = 0; n < (int) vFiles.size(); n++) {
        ...
        LOG_WRITE("SMBReader Paste sTargetName 1 [%s] [%s] [%s]", sSourceName.c_str(), sTargetName.c_str(),
                  _sCurPath.c_str());
...
bool SMBReader::Remove(MLS::Selection &tSelection, bool bMsgShow, bool bIgnore) {
    _pProgress->init();
    _pProgress->setTitle(_("samba delete file"));
