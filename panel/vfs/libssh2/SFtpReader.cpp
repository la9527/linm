...
bool SFtpReader::Next() {
    ...
    int nRt = libssh2_sftp_readdir((LIBSSH2_SFTP_HANDLE *) _pDir, cFileName, 2047,
                                   (LIBSSH2_SFTP_ATTRIBUTES *) _pFileAttr);
        ...
        if (out) {
            for (;;) {
                ...
                if (pFile->uSize && uCsize && uPsize && uFileSize) {
                    _pProgress->setOneValue(uCsize * 100 / pFile->uSize);
...
bool SFtpReader::Paste(Selection &tSelection) {
    ...
    for (int n = 0; n < (int) vFiles.size(); n++) {
        ...
        LOG_WRITE("SFtpReader Paste sTargetName 1 [%s] [%s] [%s]", sSourceName.c_str(), sTargetName.c_str(),
                  _sCurPath.c_str());
        ...
        pFtpOutFileHandle = libssh2_sftp_open((LIBSSH2_SFTP *) _pSessionSFtp, (char *) sTargetName.c_str(),
                                              LIBSSH2_FXF_READ | LIBSSH2_FXF_WRITE | LIBSSH2_FXF_TRUNC |
                                              LIBSSH2_FXF_CREAT,
                                              0644);
...
bool SFtpReader::Remove(MLS::Selection &tSelection, bool bMsgShow, bool bIgnore) {
    _pProgress->init();
    _pProgress->setTitle(_("sftp delete file"));
...
bool SFtpReader::View(const File *pFileOriginal, File *pFileChange) {
    ...
    _pProgress->init();
    _pProgress->setTitle(_("sftp view file"));

    _pProgress->setLeftStr(pFileOriginal->sName.c_str());
    _pProgress->showProgBar();;
    ...
    if (out) {
        for (;;) {
            ...
            _pProgress->setOneValue(uCsize * 100 / uFileSize);
        }
    } else {
        libssh2_sftp_close_handle(pFtpFileHandle);
        _pProgress->hideProgBar();;
