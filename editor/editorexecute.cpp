...
void	Editor::Key_End()
{
	_nCurCulumn = _vText[_nCurLine].wLine.size();
	_EditSelect.x2 = _nCurCulumn;
	_EditSelect.y2 = _nCurLine;
	_nCurCulumn_Max = _nCurCulumn;
	if (_EditMode == SHIFT_SELECT) _EditMode = EDIT;
...
void	Editor::Key_Left()
{
	if (_nCurCulumn > 0)
	{
		...
		for (int n = 0; n < _nTabSize; n++)
		{
			...
			nCul++;
			sWStr = _vText[_nCurLine].wLine.substr(_nCurCulumn, 1);
...
{
	wstring sWStr = _vText[_nCurLine].wLine;
	...
	if ((int)sWStr.size() > _nCurCulumn)
	{
		...
		{
			_nCurCulumn = _nCurCulumn + 1;
			sWTabStrChk = _vText[_nCurLine].wLine.substr(_nCurCulumn, 1);
			if (_nCurCulumn >= (int)_vText[_nCurLine].wLine.size() || wstrtostr(sWTabStrChk) != sStr) break;
			if ( n == 0 )
			{
				sWTabStrChk = _vText[_nCurLine].wLine.substr(_nCurCulumn-1, 1);
				if (wstrtostr(sWTabStrChk) != sStr) break;
			}
		}
	}
	else if ((int)sWStr.size() == _nCurCulumn && _nCurLine != _vText.size()-1 )
	{
		_nCurLine++;
		_nCurCulumn = 0;
	}
...
void	Editor::Key_Up()
{
	...
	// if over the end point charater of the line, go to the end point.
	int nStrlen = _vText[_nCurLine].wLine.size();
...
void	Editor::SelectAll()
{
	...
	_EditSelect.x2 = _vText[_vText.size()-1].wLine.size();
...
void 	Editor::Copy()
{
	...
	{
		WStr = _vText[_EditSelect.y1].wLine;
...
void	Editor::Selected_Del()
{
	...
	else
	{
		...
		WStr1 = _vText[_EditSelect.y1].wLine;
		WStr2 = _vText[_EditSelect.y2].wLine;
		...
		for (int y = _EditSelect.y1; y <= _EditSelect.y2; ++y)
		{
			...
			{
				vSave.push_back(_vText[_EditSelect.y1+1].wLine );
				_vText.erase(_vText.begin()+_EditSelect.y1+1);
			}
		}
...
void 	Editor::Paste()
{
	...
	if (vClip.size() == 1)
	{
		...
		// data save for undo
		_vDoInfo.push_back(new DoInfo(_nCurLine, _nCurCulumn, _vText[_nCurLine].wLine));

		_vText[_nCurLine].wLine = WClip2;
	...
	{
		// data save for undo
		_vDoInfo.push_back(new DoInfo(_nCurLine, _nCurCulumn, vClip.size(), _vText[_nCurLine].wLine));
        ...
        PostUpdateLines( _nCurLine, vClip.size() + 1 );
	}
...
void	Editor::Undo()
{
	...
	if (pDoInfo->nDelSize == -1) // paste. (Tab)
	{
		for (int n = 0; n < (int)pDoInfo->vData.size(); n++)
		{
			_vText[nLine+n].wLine = pDoInfo->vData[n];
		}
		PostUpdateLines( nLine, pDoInfo->vData.size() );
	}
	else if (pDoInfo->nDelSize == 0) // removed data (Insert)
	{
		for (int n = 0; n < (int)pDoInfo->vData.size(); n++)
		{
			if (n == 0)
				_vText[nLine].wLine = pDoInfo->vData[n];
			else
...
bool	Editor::FileSave()
{
	...
	{
		_nConvInfo = _vDoInfo.size(); // save the save point. check the save file.
		return true;
	}
	return false;
}
...
void	Editor::FindPrevious()
{
	...
	for(;;)
	{
		...
		{
			wstr = _vText[n].wLine.substr(0, _nFindPosX);
			...
			if (n > 0)
				_nFindPosX = _vText[n-1].wLine.size();
		}
